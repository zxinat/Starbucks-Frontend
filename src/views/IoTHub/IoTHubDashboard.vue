...
export default {
  ...
  methods: {
    ...
    async handleDelete(index, data) {
       console.log(data)
       var res = await deleteIoThub(this.username,this.subscriptionId,data.resourcegroup, data.name)
        if (res.data == "success") {
        this.$message({
          message: "已删除",
          type: "success"
        });
        this.displayData = (await listbysubid(this.username,this.subscriptionId)).data;
      } else {
    ...
    async onSubmit() {
      ...
      var res = await createIoThub(this.username,this.subscriptionId,this.form.resourceGroup, this.form.name, {
        name: this.form.name,
  ...
  async mounted() {
    ...
    this.resourcegroup = (await resourcegroup(this.username,this.subscriptionId)).data.value;
  }
};
</script>
