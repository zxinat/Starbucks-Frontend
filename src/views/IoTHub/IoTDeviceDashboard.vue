...
<script>
import { getDevices, createIotDevice, deleteDevice } from "@/api/api.js";
export default {
  ...
  methods: {
    ...
    async handleCreateDevice() {
       var res = await createIotDevice(this.createDevice.deviceId,  {
        hostName: this.$store.state.hostName,
        sharedAccessKeyName: this.$store.state.accessKey.keyName,
        sharedAccessKey: this.$store.state.accessKey.primaryKey
      });
      if (res.data == "success") {
        ...
        this.displayData = (await getDevices(this.username,this.$props)).data;
      } else {
  ...
  async mounted() {
    this.tableData = ((await getDevices(this.username,this.$props)).data.body);
